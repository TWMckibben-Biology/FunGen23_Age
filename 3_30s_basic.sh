...
CLEAND=/scratch/$MyID/FinalProject/CleanData2                      ## Example:/scratch/$MyID/PracticeRNAseq/CleanData20   #   *** This is where the cleaned paired files are located
REFD=/scratch/$MyID/FinalProject/GRCm39      ## Example:/scratch/$MyID/PracticeRNAseq/DaphniaRefGenome_6                      # this directory contains the indexed reference genome for the garter snake
MAPD=/scratch/$MyID/FinalProject/Map_HiSat2_6            ## Example:/scratch/$MyID/PracticeRNAseq/Map_HiSat2_6
COUNTSD=/scratch/$MyID/FinalProject/Counts_StringTie_6   ## Example:/scratch/$MyID/PracticeRNAseq/Counts_StringTie_6

RESULTSD=/home/aubclsb0323/FunGen23_Age/Counts_H_S_6     

REF=GRCm39_basic                  ## This is what the "easy name" will be for the genome reference
...
###Don't need these since we have those files in our reference directory
#cp /scratch/aubclsb0323/FinalProject/GRCm39/$REF.fasta .
#cp /scratch/aubclsb0323/FinalProject/GRCm39/$REF.gtf .  ### was originally gff3

###  Identify exons and splice sites

###Don't need this since we are already using a gtf file
...
cd $CLEAND  
cp /scratch/$MyID/FinalProject/CleanData/30s/*_paired.fastq .
...
mkdir "$COUNTSD"/"$i"
stringtie -p 3 -e -B -G  "$REFD"/"$REF".gtf -o "$COUNTSD"/"$i"/"$i".gtf -l "$i"   "$MAPD"/"$i"_sorted.bam
