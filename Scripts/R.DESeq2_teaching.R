...
###DO THESE ONE AT A TIME. YOU HAVE TO CONFIRM YOU WANT TO UPDATE THEM INDIVIDUALLY
...
library(DESeq2)
library(pheatmap)
###THIS IS WHERE YOU SET THE WORKING DIRECTORY TO WHERE YOU DOWNLOADED THE FILES
setwd("C:/FunGen/Assignments/FinalProject")
getwd
...
###Or you can use this
countdata = as.matrix(read.csv(file.choose(), row.names = "X"))
dim(countdata)
head(countdata)
...
coldata <-(read.csv("PHENO_DATA.txt", header=TRUE, row.names=1))
#OR use this to manually load in data
...
###These should both come out as TRUE

## Create the DESEQ dataset and define the statistical model (page 6 of the manual)

dds <- DESeqDataSetFromMatrix(countData = countdata, colData=coldata,  design = ~Age)
#look at it
#dds
...
dds$condition <- factor(dds$Age, levels=c("Postnatal_month_8","Postnatal_month_18", "Postnatal_month_22", "Postnatal_month_24", "Postnatal_month_26", "Postnatal_month_28"))
...
##Sarah Grace, this is the section that makes your table
...
sink("MouseSarcopenia_GeneXadjp.csv")
on.exit(close(sink))
  res05$padj
  
###    1.5.1 MA-plot
##This doesn't apply to anyone, but we could probably throw it into the presentation
  ## Question 4
...
##  1.5.2 Plot counts - sanity check!
  ...
  #rld <- rlog(dds) Takes forever on this many samples, vst is better
...
#Hannah, this is your section

  sampleDists <- dist(t(assay(vst)))
  library("RColorBrewer")
  
png(filename = "MouseSarcopenia_AgeHeatmap.png")
  sampleDistMatrix <- as.matrix(sampleDists)
  rownames(sampleDistMatrix) <- paste(vst$Age)
...
##Abbey, this one is for you
png(filename = "MouseSarcopenia_PCA.png")
    plotPCA(vst, intgroup=c("Age"))
dev.off()

# 2.2.4 Time series analysis
##Taylor, this one is your section